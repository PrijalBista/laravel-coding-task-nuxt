<template>
	...
	<div class="row">
	    <div class="col-lg-12">
	        <h1 class="mb-5">Company / Edit <small><span class="badge badge-info" @click="$router.back()"><i class="las la-arrow-left"></i>Back</span></small></h1>
...
<script>
	export default {
		...
		created() {
			this.$axios
				.get(`/api/company/${this.$route.params.company}`)
				.then(({ data }) => {
		...
		methods: {
			submit() {
				this.$axios
					.put(`/api/company/${this.$route.params.company}`, this.data)
					.then(({data})=> {
